...
const commands: Commands = {
  ...
  temperature: {
    ...
    run: async (
      ...
      const temperatureStr: string = await commandArgument(
        runtime,
        args.shift(),
        usage
      );
      if (Number.isNaN(Number(temperatureStr))) {
        return fail(
          `temperature: expected number in [0..2], got ${temperatureStr} (${typeof temperatureStr})`
        );
      }
      const temperature = Number(temperatureStr);
      if (temperature < 0 || temperature > 2) {
        return fail(
          `temperature: expected number in [0..2], got ${temperature}`
        );
      }
